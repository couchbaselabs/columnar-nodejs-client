...
struct js_to_cbpp_t<std::chrono::nanoseconds> {
  static inline Napi::Value to_js(Napi::Env env, const std::chrono::nanoseconds& cppObj)
  {
  ...
  static inline std::chrono::nanoseconds from_js(Napi::Value jsVal)
  {
    ...
    return std::chrono::duration_cast<std::chrono::nanoseconds>(jsVal.ToNumber().Int64Value() *
...
struct js_to_cbpp_t<std::chrono::microseconds> {
  static inline Napi::Value to_js(Napi::Env env, const std::chrono::microseconds& cppObj)
  {
    return Napi::Number::New(env, cppObj / std::chrono::milliseconds(1));
  }
  ...
  static inline std::chrono::microseconds from_js(Napi::Value jsVal)
  {
    ...
    return std::chrono::duration_cast<std::chrono::microseconds>(jsVal.ToNumber().Int64Value() *
                                                                 std::chrono::milliseconds(1));
  }
};
...
struct js_to_cbpp_t<std::chrono::milliseconds> {
  static inline Napi::Value to_js(Napi::Env env, const std::chrono::milliseconds& cppObj)
  {
    return Napi::Number::New(env, cppObj / std::chrono::milliseconds(1));
  }
...
struct js_to_cbpp_t<std::chrono::seconds> {
  static inline Napi::Value to_js(Napi::Env env, const std::chrono::seconds& cppObj)
  {
    return Napi::Number::New(env, cppObj / std::chrono::milliseconds(1));
  }
  ...
  static inline std::chrono::seconds from_js(Napi::Value jsVal)
  {
    ...
    return std::chrono::duration_cast<std::chrono::seconds>(jsVal.ToNumber().Int64Value() *
                                                            std::chrono::milliseconds(1));
  }
};
...
struct js_to_cbpp_t<std::optional<T>> {
  static inline Napi::Value to_js(Napi::Env env, const std::optional<T>& cppObj)
  {
    if (!cppObj.has_value()) {
      return env.Undefined();
    }
...
struct js_to_cbpp_t<std::array<T, N>> {
  static inline Napi::Value to_js(Napi::Env env, const std::array<T, N>& cppObj)
  {
    auto jsArr = Napi::Array::New(env);
    for (auto i = 0; i < N; ++i) {
      jsArr.Set(i, cbpp_to_js<T>(env, cppObj[i]));
...
struct js_to_cbpp_t<std::map<std::string, T, Args...>> {
  static inline Napi::Value to_js(Napi::Env env, const std::map<std::string, T, Args...>& cppObj)
  {
...
struct js_to_cbpp_t<std::map<K, T>, typename std::enable_if_t<std::is_enum_v<K>>> {
  static inline Napi::Value to_js(Napi::Env env, const std::map<K, T>& cppObj)
  {
    using enum_type_t = std::underlying_type_t<K>;
    auto jsObj = Napi::Object::New(env);
    for (const auto& i : cppObj) {
      jsObj.Set(std::to_string(static_cast<enum_type_t>(i.first)), cbpp_to_js<T>(env, i.second));
    }
    return jsObj;
  }
...
struct js_to_cbpp_t<std::variant<Types...>> {
  template<size_t I = 0>
  static inline Napi::Value to_js(Napi::Env env, const std::variant<Types...>& cppObj)
  {
...
struct js_to_cbpp_t<std::vector<std::byte>> {
  static inline Napi::Value to_js(Napi::Env env, const std::vector<std::byte>& cppObj)
  {
